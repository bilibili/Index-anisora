...
def sp_attention_forward(layer, hidden_states, mask, **kw_args):
    ...
    if pad > 0:
        query_layer=query_layer[:,:,:-pad]
        key_layer=key_layer[:,:,:-pad]
        value_layer=value_layer[:,:,:-pad]

    context_layer = attention_fn(query_layer, key_layer, value_layer, mask, dropout_fn, **kw_args)

    if pad > 0:
        context_layer=F.pad(context_layer,(0,0,0,pad))
...
def fastercache_transformer_forward(self, input_ids, position_ids, attention_mask, *,
    ...
    if True:
        ...
        pad=0
        if(world_size>1):
            mod=hidden_states.shape[1]%world_size
            pad=world_size-mod
            hidden_states=F.pad(hidden_states,(0,0,0,pad))#BTC,padT
        ...
        if pad>0:
            logits=logits[:,:-pad]#torch.Size([2, 40006, 3072])#40006=13*1088*720/256+226
...
def sampling_main(args, model_cls):
    ...
    image_size = [args.model_config.network_config.params.latent_height*8, args.model_config.network_config.params.latent_width*8]
        ...
        for text, cnt in tqdm(data_iter):
            ...
            for index in range(args.batch_size):
                ...
                for i in range(loop_num):
                    ...
                    with torch.no_grad():
                        recon = model.first_stage_model.decode(latent[:, :, start_frame:end_frame].contiguous().to(device), clear_fake_cp_cache=clear_fake_cp_cache).cpu()
