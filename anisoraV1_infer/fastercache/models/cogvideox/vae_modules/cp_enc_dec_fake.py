...
class ContextParallelCausalConv3d(nn.Module):
    ...
    def fake_context_parallel_forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        if kernel_size > 1:
            cached_inputs = [self.cache_padding] if self.cache_padding is not None else [inputs[:, :, :1]] * (kernel_size - 1)
            inputs = torch.cat(cached_inputs + [inputs], dim=2)
        return inputs

    def forward(self, inputs: torch.Tensor, clear_cache=True) -> torch.Tensor:
    ...
    def _clear_fake_context_parallel_cache(self):
        if self.cache_padding!=None:
            self.cache_padding=self.cache_padding.cpu()
            del self.cache_padding
            torch.cuda.empty_cache()
            self.cache_padding = None


    def forward0(self, input_, clear_cache=True):
